...
void ruotaSingolo(){
        strip.setPixelColor(0, Color(0,0,1));
        strip.show();   // write all the pixels out
        delay(DELAY); 


        strip.setPixelColor(0, Color(0,1,0));
        strip.show();   // write all the pixels out
        delay(DELAY); 
...
void ciclaUnLed(){
  for (int r=MIN; r < MAX; r+=INCR) {
    for (int g=MIN; g < MAX; g+=INCR) {
...
void impulse(){
  for (int i=0; i < strip.numPixels(); i++) {
    ...
    strip.setPixelColor(i, Color(255-(i*2),0,2*i));

    /*
    if(i!=strip.numPixels()) strip.setPixelColor(strip.numPixels()-i+1, Color(0,0,0));
  ...
  for (int i=strip.numPixels()-1; i >=0 ; i--) {
    if(i!=strip.numPixels()) strip.setPixelColor(i+1, Color(0,0,0));
    strip.setPixelColor(i, Color(255-(i*2),0,2*i));
